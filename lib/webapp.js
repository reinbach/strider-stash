...
function validateAuth(req, accessToken, refreshToken, profile, done) {
    ...
    var account = req.user.account('stash', profile.id);

    req.user.accounts.push(makeAccount(accessToken, profile, req.user));
    req.user.save(function (err) {
        done(err, req.user);
    });
};

function makeAccount(accessToken, profile, user) {
    return {
        provider: 'stash',
        id: profile.id,
