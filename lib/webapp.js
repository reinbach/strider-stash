...
module.exports = {
    ...
    teardownRepo: function (account, config, project, done) {
        ...
        api.teardownRepo(repo.owner, repo.name, url, key, account.token, account.tokenSecret, function (err, deleted) {
            if (err) return done(err);
            return done();
        });
    },
...
function validateAuth(req, token, tokenSecret, profile, done) {
    ...
    var account = req.user.account('stash', setAccountId(req.user));
